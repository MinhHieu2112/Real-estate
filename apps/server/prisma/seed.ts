...
const { PrismaClient, Prisma } = require('../src/generated/prisma/client');
...
// 3. Truyền adapter vào options (Thỏa mãn điều kiện 1 argument của Prisma)
...
async function resetSequence(modelName: string) {
  ...
  const maxIdResult = await (
    prisma[modelName as typeof PrismaClient] as any
  ).findMany({
    select: { id: true },
    orderBy: { id: 'desc' },
    take: 1,
