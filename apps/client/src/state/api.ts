...
export const api = createApi({
  ...
  endpoints: (build) => ({
    ...
    deleteMessage: build.mutation<{ id: number; conversationId: number }, number>({
      query: (id: number) => ({ url: `messages/${id}`, method: "DELETE" }),
      invalidatesTags: ["Messages", "Conversations"],
    }),

    deleteConversation: build.mutation<{ conversationId: number }, number>({
    ...
    deleteNotification: build.mutation<Notification, number>({
      query: (id: number) => ({ url: `notify/${id}`, method: "DELETE" }),
      invalidatesTags: ["Notifications"],
    }),

    deleteAllNotifications: build.mutation<{ count: number }, void>({
      query: () => ({ url: "notify/all", method: "DELETE" }),
      invalidatesTags: ["Notifications"],
    }),
